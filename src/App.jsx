        ...
        {view === "input" && !showComparison && (
          <>
            {apartments.length === 1 && getTotal(apartments[0]) === 0 && (
              <div style={{ marginBottom: 16, padding: "18px 22px", background: "linear-gradient(135deg, #fbeaef 0%, #f6d5df 100%)", borderRadius: 14, border: "1px solid rgba(220,86,126,0.2)" }}>
                <div style={{ fontSize: 17, fontWeight: 600, color: "var(--rose-900)", marginBottom: 8 }}>Welcome to RentWise ✨</div>
                <div style={{ fontSize: 14, color: "var(--rose-800)", lineHeight: 1.65 }}>Finding your next home is exciting, and you should have a clear understanding of your real month-to-month costs for your next dream apartment! We'll help you organize, compare, and see the full picture — so you can focus on finding <em>the one</em>.</div>
              </div>
            )}
                ...
                <div key={apt.id} id={`apt-card-${apt.id}`} className="apt-card" style={{ marginBottom: 12, animationDelay: `${idx * 0.06}s` }}>
                  <div style={{ background: "#fff", borderRadius: 16, border: isCheapest ? "1.5px solid var(--green)" : isMostExpensive ? "1.5px solid var(--coral)" : "1.5px solid var(--border)", overflow: "hidden", boxShadow: isOpen ? "0 8px 32px rgba(0,0,0,0.08)" : "0 2px 8px rgba(0,0,0,0.04)" }}>
                    {(isCheapest || isMostExpensive) && (
                      <div style={{ fontSize: 11, letterSpacing: 1.8, textTransform: "uppercase", fontWeight: 600, background: isCheapest ? "var(--green-light)" : "var(--coral-light)", color: isCheapest ? "var(--green)" : "var(--coral)", padding: "5px 16px" }}>{isCheapest ? "✓  Lowest True Cost" : "⚠  Highest True Cost"}</div>
                    )}
                    ...
                    <div className={`card-body ${isOpen ? "open" : ""}`}>
                      <div style={{ padding: "22px 20px 20px" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.4fr 0.8fr 0.8fr", gap: 10, marginBottom: 10 }}>
                          ...
                          <MetaInput label="City" value={apt.city} onChange={v => update(apt.id, "city", v)} placeholder="e.g. Los Angeles" />
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "1fr 1fr 1fr 1.2fr", gap: 10, marginBottom: 22 }}>
                          ...
                          <MetaInput label="Lease (months)" value={apt.leaseterm} onChange={v => update(apt.id, "leaseterm", v)} placeholder="e.g. 12" type="number" />
                        ...
                        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                          {["costs", "movein"].map(tab => (
                            <button key={tab} onClick={() => setActiveSection(prev => ({ ...prev, [apt.id]: tab }))} style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 15, fontWeight: 500, fontFamily: "'DM Sans', sans-serif", background: section === tab ? "var(--dark)" : "var(--cream)", color: section === tab ? "#fff" : "var(--gray)" }}>
                        ...
                        {section === "costs" && (
                          <>
                            <div style={{ fontSize: 11, letterSpacing: 1.8, textTransform: "uppercase", color: "var(--rose-700)", fontWeight: 600, marginBottom: 10 }}>Monthly Costs</div>
                            {COST_CATEGORIES.map(cat => {
                              ...
                              return (
                                <div key={cat.key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: cat.key === "rent" ? "1.5px solid var(--rose-500)" : "1px solid var(--border)" }}>
                                  <span style={{ fontSize: 17, width: 26, textAlign: "center" }}>{cat.icon}</span>
                                  <label style={{ flex: 1, fontSize: 15, color: cat.key === "rent" ? "var(--dark)" : "#3a3a3a", fontWeight: cat.key === "rent" ? 600 : 400 }}>{cat.label}</label>
                                  <div style={{ position: "relative", width: 120 }}>
                                    <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#bbb", fontSize: 13 }}>$</span>
                                    <input type="number" min="0" value={apt.costs[cat.key]} onChange={e => update(apt.id, cat.key, e.target.value)} placeholder="0" style={{ width: "100%", padding: "10px 12px 10px 28px", border: cat.key === "rent" ? "1.5px solid var(--rose-500)" : "1px solid var(--border)", borderRadius: 8, fontSize: 15, fontFamily: "'DM Sans', sans-serif", background: cat.key === "rent" ? "var(--rose-50)" : "var(--warm-white)", color: "var(--dark)" }} />
                                  </div>
                            ...
                            {apt.sqft && total > 0 && <div style={{ textAlign: "right", fontSize: 15, color: "var(--rose-700)", marginTop: 4 }}>${(total / Number(apt.sqft)).toFixed(2)} per sqft/mo</div>}
                          </>
                        )}
                        ...
                        {section === "movein" && (
                          <>
                            <div style={{ fontSize: 11, letterSpacing: 1.8, textTransform: "uppercase", color: "var(--rose-700)", fontWeight: 600, marginBottom: 10 }}>Move-In Costs</div>
                            {MOVEIN_FIELDS.map(f => (
                              <div key={f.key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: "1px solid var(--rose-200)" }}>
                                <span style={{ fontSize: 17, width: 26, textAlign: "center" }}>{f.icon}</span>
                                <label style={{ flex: 1, fontSize: 15, color: "#3a3a3a" }}>{f.label}</label>
                                <div style={{ position: "relative", width: 120 }}>
                                  <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#bbb", fontSize: 13 }}>$</span>
                                  <input type="number" min="0" value={apt.movein[f.key]} onChange={e => update(apt.id, f.key, e.target.value)} placeholder="0" style={{ width: "100%", padding: "10px 12px 10px 28px", border: "1px solid var(--rose-200)", borderRadius: 8, fontSize: 15, fontFamily: "'DM Sans', sans-serif", background: "var(--warm-white)", color: "var(--dark)" }} />
                                </div>
                                <span style={{ width: 36 }} />
                              </div>
                            ))}
                            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, paddingTop: 14, borderTop: "2px solid var(--dark)" }}>
                              <span style={{ fontSize: 15, fontWeight: 600 }}>Total Move-In Cost</span>
                        ...
                        <div style={{ marginTop: 18 }}>
                          <label style={{ display: "block", fontSize: 11, letterSpacing: 1.8, textTransform: "uppercase", color: "var(--rose-700)", fontWeight: 600, marginBottom: 6 }}>Notes</label>
                          <textarea value={apt.notes} onChange={e => update(apt.id, "notes", e.target.value)} placeholder="Lease terms, agent contact, move-in timeline, vibe..." style={{ width: "100%", border: "1px solid var(--rose-200)", borderRadius: 10, padding: "12px 14px", fontSize: 15, fontFamily: "'DM Sans', sans-serif", color: "#3a3a3a", minHeight: 70, background: "var(--warm-white)", lineHeight: 1.6 }} />
                        </div>

                        {/* Photos */}
                        <div style={{ marginTop: 18 }}>
                          <div style={{ fontSize: 11, letterSpacing: 1.8, textTransform: "uppercase", color: "var(--rose-700)", fontWeight: 600, marginBottom: 6 }}>📷 Photos {apt.photos?.length > 0 && <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>({apt.photos.length})</span>}</div>
                          <div className="photo-upload-zone" onClick={() => document.getElementById(`photo-${apt.id}`).click()}>
                            ...
                            <div style={{ fontSize: 14, color: "var(--rose-700)", fontWeight: 500 }}>Click or drag photos here</div>
                            <div style={{ fontSize: 14, color: "var(--rose-600)", marginTop: 4 }}>Listing photos, in-person shots, floor plans...</div>
                          </div>
                          {apt.photos?.length > 0 && (
                        ...
                        <div style={{ display: "flex", gap: 12, marginTop: 16, flexWrap: "wrap" }}>
                          <button onClick={() => duplicateApartment(apt)} style={{ background: "none", border: "1px solid var(--rose-200)", borderRadius: 8, color: "var(--rose-700)", fontSize: 15, fontFamily: "'DM Sans', sans-serif", cursor: "pointer", padding: "6px 12px", fontWeight: 500 }}>📋 Duplicate</button>
                          {apartments.length > 1 && <button onClick={() => confirmRemoveApartment(apt)} style={{ background: "none", border: "none", color: "var(--coral)", fontSize: 15, fontFamily: "'DM Sans', sans-serif", cursor: "pointer", fontWeight: 500 }}>✕ Remove</button>}
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="action-btn" onClick={addApartment} style={{ flex: 1, padding: 14, border: "1.5px dashed var(--border)", borderRadius: 14, background: "transparent", cursor: "pointer", color: "var(--rose-700)", fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><span style={{ fontSize: 18 }}>+</span> Add Apartment</button>
              {apartments.length > 1 && <button className="action-btn" onClick={clearAll} style={{ padding: "14px 16px", border: "1.5px solid var(--rose-200)", borderRadius: 14, background: "transparent", cursor: "pointer", color: "var(--rose-700)", fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Reset All</button>}
            </div>
          </>
        )}
...
function MetaInput({ label, value, onChange, placeholder, type = "text" }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 11, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--rose-700)", fontWeight: 600, marginBottom: 6 }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} style={{ display: "block", width: "100%", border: "1px solid var(--rose-200)", borderRadius: 9, padding: "12px 14px", fontSize: 15, fontFamily: "'DM Sans', sans-serif", color: value ? "var(--dark)" : "var(--gray)", background: "var(--warm-white)" }} />
    </div>
  );
}
